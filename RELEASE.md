...
Placeholder for `pycosat MAJOR.MINOR.x` release.
...
- [ ] Ensure release candidates are being successfully built (see `conda-canary/label/rc-pycosat-MAJOR.MINOR.x`)
...
- [ ] Create Zulip thread on [#releases][zulip]
    - [ ] Announce `MAJOR.MINOR.MINOR` in-progress
...
#### Append to existing 'Release `MAJOR.MINOR.x`' issue:
```markdown
<details open>  <!-- feel free to remove the open attribute once this section is completed -->
